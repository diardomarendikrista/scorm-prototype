...
| **`userId`**       | `string`  | **Wajib**  | ID unik untuk pengguna, digunakan sebagai kunci utama untuk menyimpan progres. (Optional)                                                                                 |
...
3.  **Memuat Progres**: `useScormProgress` membaca `localStorage` untuk mencari data progres yang cocok dengan `courseId` dan `userId`. Jika ada, _state_ progres akan diatur, dan player akan membuka SCO terakhir yang diakses pengguna.
